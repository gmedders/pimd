...
rpmd_necklace::rpmd_necklace() : m_ndofs(0), m_nbeads(0) {}
...
void rpmd_necklace::setup(size_t ndim, size_t natom, size_t nbead, double beta,
  ...
  for (size_t k = 0; k < nbead; ++k) {
    if (k == 0) {
      ...
      for (size_t j = 0; j < nbead; ++j)
        m_cart_to_nm(j, k) = std::sqrt(1.0 / nbead) * std::pow(-1.0, j);
...
void rpmd_necklace::pos_c2n() { m_pos_nmode = m_pos_cart * m_cart_to_nm; }

//----------------------------------------------------------------------------//

void rpmd_necklace::pos_n2c() { m_pos_cart = m_pos_nmode * m_cart_to_nm.t(); }

//----------------------------------------------------------------------------//

void rpmd_necklace::mom_c2n() { m_mom_nmode = m_mom_cart * m_cart_to_nm; }
