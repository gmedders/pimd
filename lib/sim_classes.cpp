...
void pimd::calc_pos_stats(void) {
  ...
  m_L1_cart_pos = 0;
  for (size_t n = 0; n < nbeads(); ++n) {
    double diff = m_pos_cart[n * ndofs()] - m_avg_cart_pos;
...
void pimd::dump_1D_frame(std::ostream &of_traj) {
  vel_n2c();
  for (size_t n = 0; n < nbeads(); ++n) {
    of_traj << m_potential->state_id[n] << ' ';
    for (size_t i = 0; i < ndofs(); ++i) {
      of_traj << ' ' << m_pos_cart[n * ndofs() + i] << ' '
              << m_vel_cart[n * ndofs() + i];
    }
    of_traj << std::endl;
  }
}
...
void pimd::set_up(const size_t ndim, const size_t natom, const size_t nbead,
...
double pimd::force(size_t ndim, size_t natom, size_t nbeads, const double *x,
                   double *f) {
