...
bool gcmc::calc_insertion_probability() { return true; }
...
void gcmc::step(double dt, double beta) {
  ...
  if (insert) {
    ...
    m_md_ensemble->set_up(ndim, new_natoms, nbeads, beta, dt, new_crd, new_vel);
...
void gcmc::dump(std::ostream &os) {
  os << m_md_ensemble->ndim() << ' ' << m_md_ensemble->natoms() << ' '
