                          ...
                          double dt = 1.0, double mass = 2000.0)
{
    ...
    for(size_t k = 0; k < nbead; ++k) {
        if(k == 0) {
            for(size_t j = 0; j < nbead; ++j)
                m_cart_to_nm(k, j) = std::sqrt(1.0/nbead);
    ...
    for(size_t k = 1; k < nbead; ++k) {
        ...
        m_freerp_propagator(0,1,k) = -mw * s;
        m_freerp_propagator(1,0,k) = 1.0/mw * s;
...
{
    m_pos_nmode = m_cart_to_nm * m_pos_cart;
}
...
{
    m_pos_cart = m_cart_to_nm.t() * m_pos_nmode;
}
...
{
    m_mom_nmode = m_cart_to_nm * m_mom_cart;
}
...
{
    m_mom_cart = m_cart_to_nm.t() * m_mom_nmode;
}
