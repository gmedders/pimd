...
int main(int argc, char **argv) {
  ...
  if (argc < 6) {
    if (my_rank == 0)
      std::cerr << "usage: ensemble_tcf_rpmd input_file dt GammaEl gammaTh_fac "
                   "voltage time"
                << std::endl;
    return EXIT_FAILURE;
  }
  ...
  while (!ifs.eof()) {
...
#ifdef ENABLE_MPI
    // std::cerr << "Rank " << my_rank << " running frame " << iframe <<
    // std::endl;
#endif
    ...
    try {
      sim.set_up(ndim, natom, nbead, beta, dt, &all_bead_crd[0],
                 &all_bead_vel[0]);
  ...
  MPI_Allreduce(MPI_IN_PLACE, &sum_pos[0], sum_pos.size(), MPI_DOUBLE_PRECISION,
  ...
  if (my_rank == 0) {
    ...
    for (size_t i = 0; i < tcf.size(); ++i) {
      std::cout << std::setw(20) << time[i] << std::setw(20)
